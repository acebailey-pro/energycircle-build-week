...
function PageFooter({ document }: { document: EngineeringPackage }) {
  return <footer className="report-page__footer"><span>EnergyCircle pre-build field package</span><span>{document.fingerprint} · Revision {document.revision}</span></footer>;
}
  ...
  return <article className="print-report" aria-hidden="true">
    ...
    <section className="report-page">
      ...
      <div className="report-two-column">
        ...
        <section><h3>Planning cost envelope</h3><div className="report-cost"><span>Accessible / owner-directed</span><strong>{money(result.cost.accessible.low)}–{money(result.cost.accessible.high)}</strong></div><div className="report-cost"><span>Professionally installed</span><strong>{money(result.cost.installed.low)}–{money(result.cost.installed.high)}</strong></div><p className="report-note">{result.cost.basis} Source: {result.cost.sourceLabel}, {result.cost.sourceYear}. Incentives excluded.</p></section>
      </div>
    ...
    <section className="report-page">
      ...
      <table className="report-table report-table--budget"><thead><tr><th>Assembly</th><th>Qty.</th><th>Accessible</th><th>Installed</th></tr></thead><tbody>{document.budget.map((line) => <tr key={line.id}><td><strong>{line.item}</strong><br /><small>{line.basis}</small></td><td>{line.quantity} {line.unit}</td><td>{money(line.accessible.low)}–{money(line.accessible.high)}</td><td>{money(line.installed.low)}–{money(line.installed.high)}</td></tr>)}</tbody><tfoot><tr><th>Current planning envelope</th><td /><th>{money(result.cost.accessible.low)}–{money(result.cost.accessible.high)}</th><th>{money(result.cost.installed.low)}–{money(result.cost.installed.high)}</th></tr></tfoot></table>
    ...
    {document.assemblies.map((assembly, assemblyIndex) => <section className="report-page" key={assembly.componentId}>
      <header className="report-heading"><span>05A-{String(assemblyIndex + 1).padStart(2, "0")} / Exploded assembly</span><h2>{assembly.componentLabel}</h2><p>{assembly.variantLabel} · {assembly.componentKind} · service-life evidence: {assembly.serviceLife}</p></header>
